...
type ListOffsetsRequest struct {
	// Version defines the protocol version to use for encode and decode
	Version int16
	// ReplicaID contains the broker ID of the requester, or -1 if this request is being made by a normal consumer.
	ReplicaID int32
	...
	// TimeoutMs contains the timeout to await a response in milliseconds for requests that require reading from remote storage for topics enabled with tiered storage.
	TimeoutMs int32
}
